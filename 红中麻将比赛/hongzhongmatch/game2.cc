...
int Game::DispatchJson2(Client *client)
{
	...
	if (cmd < 0) {
		log.error("packet safe check error\n");
		return -1;
	}

	Json::Value &val = client->packet.tojson();
	log.info("%s:%d:%s client fd:%d cmd:%d\n", __FILE__, __LINE__, __FUNCTION__, client->fd, cmd);
	int matchid = 0;
	try {
	...
	try
	{
		...
		case SERVER_JOIN_MATCH_SUCC:
			JoinMatchSuccAck2(client, client->packet.tojson());
			return 0;
...
int Game::DelPlayer2(Player *player, int code)
{
	...
	if (player->table) {
		...
		log.info("%s:%d:%s del tid:%d table:%p\n", __FILE__, __LINE__, __FUNCTION__, table->tid, table);
	}
	
	log.info("%s:%d:%s player:%p uid:%d\n", __FILE__, __LINE__, __FUNCTION__, player, player->uid);
	delete player;	
	return 0;
}
...
//平台验证进入比赛成功返回
void Game::JoinMatchSuccAck2(Client* org_client, Json::Value &val)
{
	...
	//验证登录失败
	if (tmpCode == 0) {
		map<int, Client*>::iterator iter = uid_client_map.find(org_client->fd);
	...
	if (ret < 0) {
		log.error("get minfo[%d] error\n", tmpMid);
		return;
	}
	if (zjh.main_rc[0]->reply->str == NULL) {
		log.error("get minfo[%d] str error\n", tmpMid);
		return;
	}
	...
	if (reader.parse(strminfo, mval) < 0) {
		log.error("parse minfo[%d] error\n", tmpMid);
		return;
	}
	log.debug("get redis minfo[%s]\n", mval.toStyledString().c_str());
	...
	if (player == NULL)
	{
		...
		if (ret < 0) {
			log.error("init player error\n");
			delete player;
			return;
		}
		log.info("%s:%d:%s one player uid:%d tid:%d new succ\n", __FILE__, __LINE__, __FUNCTION__, tmpUid, player->tid);
	}
	else
	{
		player->ResetMoney();
	...
	if (ret < 0) {
		log.error("%s player:%d join match error\n", __FUNCTION__, tmpUid);		
		Client::pre_destroy(client);		
		return;
	}
	...
	if (ret < 0){
		log.error("%s player:%d join match %d error\n", __FUNCTION__, tmpUid);
		Client::pre_destroy(client);
	}		
}
