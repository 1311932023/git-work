...
public:
	...
	MJ_BUFFER(const MJ_BUFFER<TYPE, SIZE>& other) : len(other.len)
	{		
	...
	int Max()const { return len; }
	TYPE* Front() { return cards; }
	bool FindLast(const TYPE& other, int c)
	{
		if (len == 0)
			return false;
		TYPE* end = &cards[len - 1];
		TYPE* begin = cards;
		begin = MAX((end - c), begin);
		do		
		{
			if (*end == other)
				return true;
...
typedef MJ_BUFFER<_uint32, 6>			MJ_qishou_fan;
#endif