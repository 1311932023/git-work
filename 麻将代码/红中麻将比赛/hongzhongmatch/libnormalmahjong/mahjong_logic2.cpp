...
bool CGameLogic::IsYiTiaoLongEx(const HandCards* p)
{
	_uint8 type_count[MJ_TYPE_COMMON] = { 0 };
	GetCardTypeCount(p, type_count);
	int suit = 0;
	for (; suit < 3;suit++)
	{
		if (type_count[suit] >= 9)
		{
			goto __YITIAOLONG_END__;
		}			
	}
	return false;
__YITIAOLONG_END__:
	...
	for (int i = 1; i < 10;i++)
	{
		if (!ChuPai(&tmp, MakeCard(suit, i)))
		{
			return false;
		}
	}
	MJ_win_pattern pattern;
	return FormatCards(&tmp, pattern);	
}
bool CGameLogic::L_IsYiTiaoLong(const HandCards* p, _uint8 CardDataNum[MJ_TYPE_COMMON][10], int iLaizi)
{
	return false;
}
...
bool CGameLogic::L_IsQiDui_03_Special(const HandCards* p, _uint8 c, _uint8 CardDataNum[MJ_TYPE_COMMON][10], int nHunNum, int& iLongQiDui)
{
	int nNeedHun = 0;
	_uint8 last = INVALID_MJ_CARD;
	for (int i = 0; i < p->ChangeableCardsLen; i++)
...
bool CGameLogic::L_IsQiDui_04(const HandCards* p, _uint8 c, _uint8 CardDataNum[MJ_TYPE_COMMON][10], int nHunNum, int& iLongQiDui)
{
	if (p->FixedCardsLen != 0)
	{
		return false;
...
int CGameLogic::GetCardCount(const HandCards* pHc, CardNode_set& h)
{
	h.Clear();
	if (pHc->ChangeableCardsLen == 0)
		return h.Size();
	h.Add(CardNode_count(pHc->ChangeableCards[0], 1));
	for (size_t i = 1; i < pHc->ChangeableCardsLen; i++)
	{
		CardNode_count& last = h.Last();
		if (last.card == pHc->ChangeableCards[i])
		{
			last.count++;
		}
		else
		{
			h.Add(CardNode_count(pHc->ChangeableCards[i], 1));
		}
	}
	return h.Size();
}
...
bool CGameLogic::Is_DanDiaoJiang(const HandCards* pHc, _uint8 bHuCard, const MJ_win_pattern& oPattern)
{
	if (oPattern.jiang == bHuCard)
	{
		std::set<_uint8> checked;
		checked.insert(bHuCard);

		HandCards tmp = *pHc;
		ChuPai(&tmp, bHuCard);		
		for (int i = 0; i < tmp.ChangeableCardsLen; i++)
		{
			_uint8 card[5] = { tmp.ChangeableCards[i] - 2,
				tmp.ChangeableCards[i] - 1,
				tmp.ChangeableCards[i],
				tmp.ChangeableCards[i] + 1,
				tmp.ChangeableCards[i] + 2 };

			for (int j = 0; j < 5; j++)
			{
				if (CardNum(card[j]) < 10 && CardNum(card[j]) > 0)
				{
					if (checked.find(card[j]) != checked.end())
						continue;
					checked.insert(card[j]);
					AddCard(&tmp, card[j]);
					MJ_win_pattern pattern;
					if (FormatCards(&tmp, pattern))
					{
...
int CGameLogic::GetAnJian(const HandCards* pHc, _uint8 bCard)
{
	bool zhong_fa_bai[3] = { false };
	if (bCard == INVALID_MJ_CARD)
	{
		for (_uint8 i = 0; i < pHc->ChangeableCardsLen; i++)
		{
			switch (pHc->ChangeableCards[i])
			{
			case MJ_HONG_ZHONG:	zhong_fa_bai[0] = true; break;
			case MJ_FA_CAI: zhong_fa_bai[1] = true; break;
			case MJ_BAI_BAN: zhong_fa_bai[2] = true; break;
			default: break;
			}
		}
		return zhong_fa_bai[0] && zhong_fa_bai[1] && zhong_fa_bai[2] ? TYPE_PENG : TYPE_NULL;			
	}
	else if (bCard == MJ_HONG_ZHONG)
	{
		zhong_fa_bai[0] = true;		
		for (_uint8 i = 0; i < pHc->ChangeableCardsLen; i++)
		{
		...
		return zhong_fa_bai[0] && zhong_fa_bai[1] && zhong_fa_bai[2] ? TYPE_RIGHT_CHI : TYPE_NULL;			
	}
	else if (bCard == MJ_FA_CAI)
	{
		zhong_fa_bai[1] = true;
		for (_uint8 i = 0; i < pHc->ChangeableCardsLen; i++)
		{
		...
		return zhong_fa_bai[0] && zhong_fa_bai[1] && zhong_fa_bai[2] ? TYPE_CENTER_CHI : TYPE_NULL;			
	}
	else if (bCard == MJ_BAI_BAN)
	{
		zhong_fa_bai[2] = true;
		for (_uint8 i = 0; i < pHc->ChangeableCardsLen; i++)
		{
		...
		return zhong_fa_bai[0] && zhong_fa_bai[1] && zhong_fa_bai[2] ? TYPE_LEFT_CHI : TYPE_NULL;
			
	}
	return TYPE_NULL;
}

void CGameLogic::ExecuteAnJian(HandCards* pHc, int iOpType, _uint8 ChairID, _uint8 bCard)
{
	bool r = false;
	if (bCard == INVALID_MJ_CARD)
	{
		_uint8 cbRemoveCard[] = { MJ_HONG_ZHONG, MJ_FA_CAI, MJ_BAI_BAN};
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;	
	}
	else if (bCard == MJ_HONG_ZHONG)
	{
		_uint8 cbRemoveCard[] = { MJ_FA_CAI, MJ_BAI_BAN };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;		
	}
	else if (bCard == MJ_FA_CAI)
	{
		_uint8 cbRemoveCard[] = { MJ_HONG_ZHONG, MJ_BAI_BAN };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;
	}
	else if (bCard == MJ_BAI_BAN)
	{
		_uint8 cbRemoveCard[] = { MJ_HONG_ZHONG, MJ_FA_CAI };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;
	}
	if (r)
	{
		HandCards* handcard = pHc;
		handcard->FixedCards[handcard->FixedCardsLen].c[0] = MJ_HONG_ZHONG;
		handcard->FixedCards[handcard->FixedCardsLen].c[1] = MJ_FA_CAI;
		handcard->FixedCards[handcard->FixedCardsLen].c[2] = MJ_BAI_BAN;
		handcard->FixedCards[handcard->FixedCardsLen].state = iOpType;
		handcard->FixedCards[handcard->FixedCardsLen].chairID = ChairID;
		handcard->FixedCardsLen++;
	}
}
int CGameLogic::GetXuanFengGang(const HandCards* pHc, _uint8 bCard)
{
	bool xuan_feng_gang[4] = { false };
	if (bCard == INVALID_MJ_CARD)
	{
		for (_uint8 i = 0; i < pHc->ChangeableCardsLen; i++)
		{
			switch (pHc->ChangeableCards[i])
			{
			case 0x31: xuan_feng_gang[0] = true; break;
			case 0x32: xuan_feng_gang[1] = true; break;
			case 0x33: xuan_feng_gang[2] = true; break;
			case 0x34: xuan_feng_gang[3] = true; break;
			default: break;
			}
		}
		return xuan_feng_gang[0] && xuan_feng_gang[1] && xuan_feng_gang[2] && xuan_feng_gang[3] ? TYPE_CAI_GANG : TYPE_NULL;			
	}
	else if (bCard == 0x31)
	{
		xuan_feng_gang[0] = true;
		for (_uint8 i = 0; i < pHc->ChangeableCardsLen; i++)
		{
			switch (pHc->ChangeableCards[i])
			{			
...
void CGameLogic::ExecuteXuanFengGang(HandCards* pHc, int iOpType, _uint8 ChairID, _uint8 bCard)
{
	bool r = false;
	if (bCard == INVALID_MJ_CARD)
	{
		_uint8 cbRemoveCard[] = { 0x31,0x32,0x33,0x34 };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;
	}
	else if (bCard == 0x31)
	{
		_uint8 cbRemoveCard[] = { 0x32,0x33,0x34 };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;
	}
	else if (bCard == 0x32)
	{
		_uint8 cbRemoveCard[] = { 0x31,0x33,0x34 };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;
	}
	else if (bCard == 0x33)
	{
		_uint8 cbRemoveCard[] = { 0x31,0x32,0x34 };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;
	}
	else if (bCard == 0x34)
	{
		_uint8 cbRemoveCard[] = { 0x31,0x32,0x33 };
		RemoveCard(pHc, (_uint8*)cbRemoveCard, sizeof(cbRemoveCard));
		r = true;
	}
	if (r)
	{
		HandCards* handcard = pHc;
		handcard->FixedCards[handcard->FixedCardsLen].CardData = 0x31;		
		handcard->FixedCards[handcard->FixedCardsLen].state = iOpType;
		handcard->FixedCards[handcard->FixedCardsLen].chairID = ChairID;
		handcard->FixedCardsLen++;
	}
