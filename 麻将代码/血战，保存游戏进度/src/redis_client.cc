...
int RedisClient::connect_redis()
{
    ...
    if (context->err) {
        log_error("redis[%s] host[%s] port[%d]\n", context->errstr, _host.c_str(), _port);
        return -1;
    }
	log_debug("redis host[%s] port[%d] is connected succ.\n", _host.c_str(), _port);
	
	int ret =  command_spec("auth %s", _pass.c_str());
	if (ret < 0) {
		log_error("auth %s >> redis[%s] host[%s] port[%d]\n", _pass.c_str(), context->errstr, _host.c_str(), _port);
	}
	return 0;
}
...
int RedisClient::command(const char *format, ...)
{
	...
	while (i--)
	{
	    ...
	    if (context->err)
	    {
	        log_error("redis[%s] ip[%s] port[%d], reconnecting...\n",
	            context->errstr, _host.c_str(), _port);
...
const char* RedisClient::get_value_as_string(const char *key)
{
    size_t i = 0;
