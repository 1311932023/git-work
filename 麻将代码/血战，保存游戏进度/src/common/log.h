...
struct tm _tm; \
	...
	snprintf(_format, sizeof(_format), "|dbug|%d-%02d-%02d %02d:%02d:%02d.%06ld %s:%s:%d %s\n", 1900 + _tm.tm_year, 1 + _tm.tm_mon, _tm.tm_mday, _tm.tm_hour, \
...
if (1)  \
{ \
	  static char info_format[10240]; \
struct timeval _tv; \
	gettimeofday(&_tv, NULL); \
struct tm _tm; \
	localtime_r(&_tv.tv_sec, &_tm); \
	snprintf(info_format, sizeof(info_format),  "|info|%d-%02d-%02d %02d:%02d:%02d.%06ld %s:%s:%d %s\n", 1900 + _tm.tm_year, 1 + _tm.tm_mon, _tm.tm_mday, _tm.tm_hour, \
	_tm.tm_min, _tm.tm_sec, _tv.tv_usec, __FILE__, __FUNCTION__, __LINE__, format); \
...
#define log_warn(format, ...) \
	if (1)  \
{ \
	static char warn_format[1024]; \
struct timeval _tv; \
	gettimeofday(&_tv, NULL); \
struct tm _tm; \
	localtime_r(&_tv.tv_sec, &_tm); \
	snprintf(warn_format, sizeof(warn_format), "|warn|%d-%02d-%02d %02d:%02d:%02d.%06ld %s:%s:%d %s\n", 1900 + _tm.tm_year, 1 + _tm.tm_mon, _tm.tm_mday, _tm.tm_hour, \
	_tm.tm_min, _tm.tm_sec, _tv.tv_usec, __FILE__, __FUNCTION__, __LINE__, format); \
	Log::Instance().my_debug(warn_format, ##__VA_ARGS__); \
}

#define log_error(format, ...) \
if (1)  \
{ \
	  static char error_format[10240]; \
struct timeval _tv; \
	gettimeofday(&_tv, NULL); \
struct tm _tm; \
	localtime_r(&_tv.tv_sec, &_tm); \
	snprintf(error_format, sizeof(error_format), "|err|%d-%02d-%02d %02d:%02d:%02d.%06ld %s:%s:%d %s\n", 1900 + _tm.tm_year, 1 + _tm.tm_mon, _tm.tm_mday, _tm.tm_hour, \
	_tm.tm_min, _tm.tm_sec, _tv.tv_usec, __FILE__, __FUNCTION__, __LINE__, format); \
...
#define log_fatal(format, ...) \
	if (1)  \
{ \
	static char fatal_format[10240]; \
struct timeval _tv; \
	gettimeofday(&_tv, NULL); \
struct tm _tm; \
	localtime_r(&_tv.tv_sec, &_tm); \
	snprintf(fatal_format, sizeof(fatal_format), "|fatal|%d-%02d-%02d %02d:%02d:%02d.%06ld %s:%s:%d %s\n", 1900 + _tm.tm_year, 1 + _tm.tm_mon, _tm.tm_mday, _tm.tm_hour, \
	_tm.tm_min, _tm.tm_sec, _tv.tv_usec, __FILE__, __FUNCTION__, __LINE__, format); \
	Log::Instance().my_debug(fatal_format, ##__VA_ARGS__); \
}
