...
struct tagListenInfos
{
	...
	tagHuInfos      HuInfo[100];         //打出牌后可以胡牌的信息
	_uint8			bHuLen;
	tagListenInfos()
	{
		bOutCard = 0;
...
public:
	...
	**/
	...
	void recv_client_checkuserhandcard(Client *client, Player *player);
...
public:
	...
	virtual void Send_GameEnd_ScoreInfo(Player*, proto::game::yunxiaomj::GameEndRecord &score);
public:	
	...
	//校验手牌
	virtual void CheckUserHandCards(_uint8 ChairID, _uint8 bCard[20],int cards_len) { return; };
...
public:
	...
	template<typename TYPE>
	void Proto_GameEndInfo_AddFan(const MJ_user_buffer& UserID, proto::game::AckGameEnd& stGameend, proto::game::yunxiaomj::tagYunXiaomjGameEnd* end, const TYPE& ft2fn);
	template<typename TYPE>
...
template<typename TYPE>
void Table::Proto_GameEndInfo_AddFan(const MJ_user_buffer& UserID, proto::game::AckGameEnd& stGameend, proto::game::yunxiaomj::tagYunXiaomjGameEnd* end, const TYPE& ft2fn)
{
	if (end == NULL)	return;
	
	proto::game::yunxiaomj::FanInfo* info[GAME_PLAYER];
	for (_uint8 i = 0; i < m_GAME_PLAYER; i++)
	{
		info[i] = end->add_faninfo();
	}	
	proto::game::yunxiaomj::AddFanInfo* pfan = NULL;	
	if (UserID.Size() == 1)
	{//一个人胡
