...
int RedisClient::init(std::string host, int port, int timeout, std::string pass, int db)
{
    _host = host;
    _port = port;
	_timeout = timeout;
...
int RedisClient::connect_redis()
{
	...
	if (m_db != 0)
	{
		ret = command_spec("select %d", m_db);
		if (ret < 0)
		{
			log.error("%s select db error %d", __FUNCTION__, m_db);
		}
	}	
	return 0;
}
