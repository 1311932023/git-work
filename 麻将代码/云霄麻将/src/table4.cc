...
void Table::Send_GameEnd_ScoreInfo(Player* player, proto::game::yunxiaomj::GameEndRecord &ger)
{ 		
	for (int i = 0; i < m_GAME_PLAYER; i++)
	{
		proto::game::yunxiaomj::ReordInfo* info = ger.mutable_userrecoreinfo(i);
...
{
	proto::game::yunxiaomj::TotalRoundScore trs;
	for (int i = 0; i < m_totalrecordinfo.round_score.Size(); i++)
	{
		proto::game::yunxiaomj::RoundScore* sc = trs.add_round();
