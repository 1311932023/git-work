...
#define TIMER_START(TIMER, STAMP)	do {ev_timer_stop(zjh.loop, &(TIMER));\
									ev_timer_set(&(TIMER), (STAMP), (0));\
									ev_timer_start(zjh.loop, &(TIMER));}while(0)

#define TIMER_STOP(TIMER)			do{ev_timer_stop(zjh.loop, &(TIMER));}while(0)

#define MaxVideoBufLen  (1024 * 500)
#define HU_RECOND_MAX	(2)
class Player;
class Client;
class Match;
...
typedef struct tagRecordLowLevelOperate
{
	...
	template<class Archive>
	void serialize(Archive & ar, const unsigned int /* file_version */) {
		printf("tagRecordLowLevelOperate::serialize\n");
		ar & iOpType;
		ar & ChairID;
		ar & bCard;
	}
...
public:	
	...
	bool operator==(const tagGangScore& other)const
	{
		return Card == other.Card && ChairID == other.ChairID && iType == other.iType;
	}
};
...
class tagGangScoreInfo 
{
	...
	template<class Archive>
	void serialize(Archive & ar, const unsigned int /* file_version */) {
		printf("tagGangScoreInfo::serialize\n");
		ar & iTotalScore;
		ar & Gang;
	}
public:
	int iTotalScore;		//杠牌输赢总分	
	MJ_BUFFER<tagGangScore, HAND_CARD_KAN_MAX> Gang; //杠牌信息
	tagGangScoreInfo()
	{
		iTotalScore = 0;	
	}
...
public:
	...
	bool operator == (const tagHuRecord& other)const
	{
...
class tagHuInfo
{
	...
	template<class Archive>
	void serialize(Archive & ar, const unsigned int /* file_version */) {
		printf("tagHuInfo::serialize\n");
		ar & iTotalScore;
		ar & stHu;
	}
public:
	_tint64	iTotalScore;	//胡牌输赢总分	
	MJ_BUFFER<tagHuRecord, HU_RECOND_MAX> stHu;
	tagHuInfo()
	{
		iTotalScore = 0;		
	}
...
//结算，玩家输赢分数
struct tagUserGangScore
{
	friend class boost::serialization::access;
	template<class Archive>
	void serialize(Archive & ar, const unsigned int /* file_version */) {
		printf("tagUserGangScore::serialize\n");
		ar & bIsNull;
		ar & uid1;
		ar & score1;
		ar & uid2;
	...
	_uint8 nGangPai;
	_uint8 nGangType;
	_uint8 nDianGangChairId;
	void init()
	{
		bIsNull = true;
		uid1 = INVALID_CHAIRID;
		score1 = 0;
		uid2 = INVALID_CHAIRID;
		score2 = 0;
		uid3 = INVALID_CHAIRID;
		score3 = 0;
		nGangPai = INVALID_MJ_CARD;
		nGangType = TYPE_NULL;
		nDianGangChairId = INVALID_CHAIRID;
		totalscore = 0;
	}
};

typedef const _uint8* (*INIT_CARD_POOL)(int round, int turn, int& size);
...
class Table
{
	...
	void save(Archive & ar, const unsigned int version) const
	{
		...
		ar & win_exp;
		ar & m_BaseScore;
		ar & send_videodata_timer_running;
		ar & req_start_timer_running;
		ar & m_conf;
		ar & m_room_owner_uid;
		ar & m_room_status;
		ar & m_totalrecordinfo;
		ar & m_GAME_PLAYER;
		ar & m_bCurOutPlayer;
		ar & m_Online_Beat;
		ar & m_Offline_timer_running;
		ar & m_RandomSeat;
		ar & m_SeatPlayer;
		ar & m_Start_Shai;
		ar & m_bGameState;
		ar & m_Outcard_Timer_running;
		ar & m_Operate_Timer_running;
		ar & m_Disbandtable_Timer0_running;
		ar & m_RecvDisbandTable_Timer_running;
		ar & m_RoundOver_timer_running;
		ar & m_NextRound_timer_running;
		ar & m_bCurPlayers;
		ar & m_Disband_ResUser;
		ar & m_GameLogic;
		ar & m_HandCard;
		ar & m_IsOffline;
		ar & m_bGangCalcImme;
		...
		ar & m_bUserOperate;
		ar & m_bCurChairID;
		ar & m_bOutCardChairID;
		ar & m_bCurOutCard;
		ar & m_bCurCard;
		ar & m_stUserOutCard;
		ar & m_bMultiOpUserNum;
		ar & m_bMultiDoneOpNum;
		ar & m_bMulitHuOpeCount;
		ar & m_bMultiOpHuNum;
		ar & m_bMulitFirstOperate;
		ar & m_bMultiUserDoneOp;
		ar & m_bMultiUserDoneOpCard;
		ar & m_PassHuPlayer;
		ar & m_RobOperateNum;
		ar & m_RobHuNum;
		ar & m_WangGangPlayer;
		ar & m_WangGangPlayerOpType;
		ar & m_RobHuDoneOp;
		ar & m_bAddFanFlag;
		ar & m_RobWanGangCard;
		ar & m_bRobWanGang;
		ar & m_bRobHu;
		ar & m_bRobGangHuJudge;
		ar & m_bRecordMultiOp;
		ar & m_bIsHu;
		ar & m_HuFanNums;
		ar & m_stGang;
		ar & m_stHuScore;
		ar & m_oFan;
		ar & m_ApplyDisbandChairid;
		ar & m_bIsDisbandTable;
		ar & m_DisbandChoose;
		ar & m_bDisbandCount;
		ar & m_bOverTimeState;
		ar & m_nPlayingTimes;
		...
		ar & m_Peng;
		ar & m_ft2fn;
		TrusteeBase *trusteePtr = m_trustee.get();
		ar & trusteePtr;
		ar & m_bChuJu;
		ar & m_GangFen;
	}
	...
	void load(Archive & ar, const unsigned int version)
	{
		...
		ar & m_Peng;
		ar & m_ft2fn;
		TrusteeBase *trusteePtr = NULL;
		ar & trusteePtr;
		m_trustee.reset(trusteePtr);
		ar & m_bChuJu;
		ar & m_GangFen;
	}
...
public:
	...
	int m_RobotRatio;
	int m_Type;	
	bool m_send_listen_info;
	bool m_game_start;
	bool m_cbInitCardPool;
	std::string m_MatchName;
public:	
	union { int tid; int roomid;};
	int vid;
	union { int zid; int gameid; };
	int							base_money;
	int                         roomGold;
	int						    stand_money;
	int							lose_exp;
...
public:
	...
	_uint8 m_bCurOutPlayer;
public://给用户广播掉线通知
	bool		m_Online_Beat[GAME_PLAYER];
	bool		m_Offline_timer_running;
...
public:
	...
	bool find(_uint8 uid);

	_uint8 SeatPlayerCount()const;
	int Sit_Down(Player *player);
	int Sit_Down(Player *player, _uint8  seatid);
	int PlayerLogin(Player *player, bool login);
	...
	//用户上桌
	int handler_uptable(Player *player, bool fixed_seat);
	//用户下桌
...
public:
	...
	**/
	...
	void recv_client_outcard(_uint8 ChairID, _uint8 bCard);
	/**
...
protected:
	...
	static void RoundOver_Timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents);
	static void NextRound_Timer_cb(struct ev_loop *loop, struct ev_timer *w, int revents);
public:		
...
protected:
	//设置操作时钟
	void set_operator_timer(_uint8 ChairID);
	//关闭操作时钟
	void stop_operator_timer(_uint8 ChairID);
	//设置出牌时钟
	void set_out_timer(_uint8 ChairID);
	float elapse_out_timer(_uint8 ChairID);	
	float elapse_operator_timer(_uint8 ChairID);

	//广播时钟剩余秒数
	void Broadcast_UpdateTimer(_tint8 ChairID);
	ev_tstamp get_operate_remain_time(int iOpType, bool bReconnnect);
	//玩家在线状态的改变
...
public:	
	...
	void save_user_get_card(_uint8 chairID, _uint8 cbCard, int nLeftCardNum, bool bGetLast = false);
...
public:
	//配置数据
	bool m_RandomSeat;	
	ev_tstamp m_bDelayTime;					 //延迟时间	
	ev_tstamp m_RobotDelayTime;	
...
public:
	...
	//出牌时钟
	bool						m_Outcard_Timer_running;
	ev_timer                    m_Outcard_Timer;
	ev_tstamp                   m_Outcard_Timer_stamp;
	//操作时钟
	bool						m_Operate_Timer_running[GAME_PLAYER];
	ev_timer                    m_Operate_Timer[GAME_PLAYER];
	ev_tstamp                   m_Operate_Timer_stamp[GAME_PLAYER];	
	
	ev_timer					m_CurTimer;	//当前开启的定时器

	//等待玩家选择是否解散房间时钟
	bool						m_Disbandtable_Timer0_running;
	ev_timer                    m_Disbandtable_Timer0;
	ev_tstamp                   m_Disbandtable_Timer_stamp;	
	ev_tstamp                   m_Disbandtable_Timer_stamp1;
	
	bool						m_RecvDisbandTable_Timer_running;
	ev_timer					m_RecvDisbandTable_Timer;
	ev_tstamp					m_RecvDisbandTable_Timer_stamp;

	bool						m_RoundOver_timer_running;
	ev_timer					m_RoundOver_timer;
	ev_tstamp					m_RoundOver_stamp;

	bool						m_NextRound_timer_running;
	ev_timer					m_NextRound_timer;
	ev_tstamp					m_NextRound_stamp;
...
protected:
	...
	MJ_peng m_LouPeng[GAME_PLAYER];//漏碰
public:
...
protected://鸟马信息
	MJ_BUFFER<_uint8, 14> m_niao;
	MJ_BUFFER<_uint8, 14> m_zhong_niao;
	_uint8 m_DaNiaoID;
protected:
...
public:
	std::auto_ptr<TrusteeBase> m_trustee;
protected:
	//听牌	
	/**
	*  @fuction: IsListening
	...
	**/	
	...
	void PackageTingInfoV(_uint8 ChairID, tagListenInfos stListenInfo[14], _uint8 iPos, void* listeninfo);
public:
...
public:
	...
	int HandlerUserOperate(int cmd, Player* player) {
		try	{
			return dispatch_client(cmd, player->client, player);
		}catch (...)		{
			CoreGameEnd();
		}
		return 0;
...
public:		
	...
	virtual int CalcMPInfo(Json::Value& mpinfo);
	/*
	 *	桌子配置初始化
	 *	平台远端发过来的配置
	 */
	...
	*/
	 ...
	 *	玩家出牌后判断其他玩家是否能够吃碰胡
	 */
	virtual int OutCardEstimate(_uint8 ChairID, _uint8 bCard, MJ_major& major, int& fan_num);
	/*
	*	玩家摸牌后判断是否能够杠听胡
	*/
...
public:
	...
	virtual void Proto_GameEndInfo_GangInfo(_uint8 ChairID, proto::game::AckGameEnd_Chengdu& stGameend);
...
public:
	...
	virtual void Calc_UserGangScore(_uint8 ChairID, int iOpType, _uint8 bCard, int iScore[GAME_PLAYER]);
	virtual void Send_GameEnd_ScoreInfo(Player*, proto::game::hunanmj::GameEndRecord &score);	
public:	
	void Send_PiaoFen(_uint8 ChairID, int iScore[GAME_PLAYER]);
	...
	void GetFirstHandCard_NoOp(_uint8 ChairID, bool bGetLast = false);	
public:
	virtual _uint8 BankerScoreAdd(_uint8 ChairID)
	{//庄家加一番
		return (ChairID == m_bBanker && m_conf.m_bZhuangXian) ? 1 : 0;
	}
	virtual _uint8 BankerScoreAdd2(_uint8 ChairID, _uint8 PaoID)
	{//庄家加一番
		return (ChairID == m_bBanker || PaoID == m_bBanker) ? 1 : 0;
	}
	...
	friend class CTrustee;
	friend class Game;
	friend class Match;

public:
	bool				m_bChuJu[GAME_PLAYER];	                      //玩家是否出局
	tagUserGangScore    m_GangFen[GAME_PLAYER][5];                    //每次杠分结算

	/*反序列化用，用于重启*/
	virtual void restart();
};

BOOST_SERIALIZATION_ASSUME_ABSTRACT(Table)
...
void Table::Proto_GameEndInfo_AddFan(const MJ_user_buffer& UserID, proto::game::AckGameEnd& stGameend, proto::game::hunanmj::tagHuNanGameEnd* end, const TYPE& ft2fn)
{
	...
	{//一个人胡
		...
		if (m_stHuScore[win_seat].stHu.IsNull()) return;
