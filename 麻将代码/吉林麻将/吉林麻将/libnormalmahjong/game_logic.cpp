...
int  CGameLogic::cur_card_count(const HandCards  *pHc  , _uint8  c  )
{
	int all_ = 0;
	for (int i = 0; i < pHc->ChangeableCardsLen;i++)
	{
		_uint8  ic = pHc->ChangeableCards[i];
		if (c == ic)
			all_++;
	}
	return all_;
...
int  CGameLogic::AddCard(HandCards *pHc, _uint8 bCard)
{ 
	...
	else
	{
		...
		{
			log_error("len---------------- %d" , pHc->ChangeableCardsLen);
			SortCard(pHc->ChangeableCards, pHc->ChangeableCardsLen);
			return -1; 
		}
...
void CGameLogic::SortCard(_uint8 p[], _uint8 len)
{
	...
	for (i=0; i<len; i++)
	{
		for (j=i+1; j<len; j++)
		{
			...
			{
				std::swap(p[i], p[j]);
				//_uint8 tempCard = p[i];//p[i] = p[j]; //p[j] = tempCard;
			}
		}		
	}
}
...
int CGameLogic::GetGangInfo_King(const HandCards*pHc, MJ_opinfo& ganginfo , int King_a , int king_b )
{
	...
	for (int i = 0; i < pHc->ChangeableCardsLen; i++)
	{
		if (king_b == pHc->ChangeableCards[i]  || pHc->ChangeableCards[i] == King_a) 
			continue; 
		...
		{
			ganginfo.Add(OpState(pHc->ChangeableCards[i], TYPE_ANGANG));   // 可以暗杠
			iOpType |= TYPE_ANGANG;
		}
	...
	for (int i = 0; i < pHc->FixedCardsLen; i++)
	{
		if (pHc->FixedCards[i].state == TYPE_PENG)
		{
			for (int j = 0; j < pHc->ChangeableCardsLen; j++)
			{
				...
				{
					ganginfo.Add(OpState(pHc->ChangeableCards[j], TYPE_WANGANG));  // 可以补杠 。
