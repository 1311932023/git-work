...
public:	
	...
	bool m_cbInitTableState;//一轮一局的时候开始的标志
...
public:
	...
	void RecvUserTrustee(Client *client, Player* player);
	//用户解散桌子请求
...
public:	
	void OnNotify(int, int, int);
public:
	//配置数据
	bool m_RandomSeat;	
	ev_tstamp m_bDelayTime;					 //延迟时间	
	ev_tstamp m_RobotDelayTime;	
	ev_tstamp m_timerout_Time;					 //延迟时间	
	bool m_auto_out_trustee;
...
public:		
	...
	virtual int ExternMpInfo(Json::Value& mpinfo, Player* player);
	/*
	 *	桌子配置初始化
	 *	平台远端发过来的配置
	 */
