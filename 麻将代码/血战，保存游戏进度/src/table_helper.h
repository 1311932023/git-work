...
enum E_POOL_MODE
{
	...
	CARD_POOL_NO_FENG_FLOWER,		//牌库剔风、花

	CARD_POOL_TTFJNoBai,	//同条风箭没有白板

	CARD_POOL_72CARDS,		//72张牌
	CARD_POOL_TIAOTONG,     //条桶
};
...
class CCardTypePool
{
	...
	template<class Archive>
	void serialize(Archive & ar, const unsigned int /* file_version */) {
		printf("CCardTypePool::serialize\n");
		ar & m_Pool;
		ar & m_MaxCount;
		ar & m_Mode;
		ar & _2inner;
...
public:		
	...
	void InitPool(_uint8 laiZi, _uint8 count, std::vector<_uint8>& pos);
