...
int Table::getTotalRound()
{
	int tround = 0;
	for (int i = 1 ; i < match->m_CurTurn; i++)
	{
		tround += match->m_TurnRoundNum[i - 1];
	}
	tround += m_CurRound;
	return tround;
}
...
{	
	...
	for (int ChairID = 0; ChairID < m_GAME_PLAYER; ChairID++)
	{
		...
		if (m_SeatPlayer[ChairID].player->m_Role == 0)
		{
			...
			r["rinfo"]["round_id"] = match->CurTurn() - 1;//match->m_CurRound;								
			r["rinfo"]["round"] = match->CurRound();
			r["rinfo"]["card_desk_id"] = match->CurRound();
			//uinfo
			...
			r["uinfo"]["card_player_id"] = //player->seatid + 1;
				player->tmp_playerid;
			r["uinfo"]["card_numerical_order"] = getTotalRound();
			...
			for (_uint8 c = NextChair(ChairID); c != ChairID; c= NextChair(c))
			{
				...
				{
					r["uinfo"]["card_rival"].append(rival->uid);
					r["uinfo"]["rival"].append(rival->uid);
				}				
			}
