...
{	
	...
	if (table)
	{
		table->m_Disbandtable_Timer0_running = false;
...
{	
	...
	m_Disbandtable_Timer0_running = true;
...
{
	log_info("%s disband tid:%d apply uid:%d\n", __FUNCTION__, tid, player->uid);
	if (m_bDeleteTable)
	{
	...
	if (!isPlaying)
	{
		log_info("uid:%d chairid:%d is not at seat, so can apply disband room \n", GetPlayerUid(player), bChairID);
...
void Table::RecvDisbandTableChoose(_uint8 ChairID, int choosestype)
{
	...
	{
		log_error("%s choosetype:%d is wrong type \n", __FUNCTION__, choosestype);
		return;
	}
	if (ChairID >= m_GAME_PLAYER)
	{
		log_error("%s chairid:%d is wrong \n", __FUNCTION__, ChairID);
		return;
	}
	...
	log_info("%s tid:%d player[%d,%d,%d,%d]choose[%d,%d,%d,%d]\n", __FUNCTION__, tid,
	...
	if (bChooseYesNum == bNeedNum)
	{
		...
		m_RecvDisbandTable_Timer_running = true;
