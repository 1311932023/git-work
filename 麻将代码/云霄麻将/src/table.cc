...
bool Table::CheckUserOutCard(_uint8 ChairID, _uint8 cbCard)
{
	...
	if (!bIsHave)
	{
		log_error("%s tid:%d uid:%d seatid:%d outcard error has no card:0x%02x  \n",  __FUNCTION__,tid, ChairID, GetPlayerUid(ChairID), cbCard);
		for (_uint8 i = 0; i < bChangeLens; i++)
		{
			log_info("tid:%d ChairID:%d card[%d]:0x%02x \n",tid, ChairID, i, m_HandCard[ChairID].ChangeableCards[i]);
		}
	}
...
void Table::save_user_get_card(_uint8 chairID, _uint8 cbCard, int nLeftCardNum)
{
	...
	//log_info("tid:%d  save_user_get_card uid:%d chairID:%d card:0x%02x getCard :%s\n",tid, GetPlayerUid(chairID), chairID, cbCard, getCard.DebugString().c_str());
...
void Table::save_user_out_card(_uint8 chairID, _uint8 cbCard)
{
	...
	//log_info("tid:%d save_user_out_card uid:%d chairID:%d card:0x%02x stUseroutcard:%s\n",tid, GetPlayerUid(chairID), chairID, cbCard, stUseroutcard.DebugString().c_str());
