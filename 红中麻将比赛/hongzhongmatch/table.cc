...
Table::Table() : m_CardPool(CARD_POOL_NO_FLOWER, 0)
{
	...
	if (zjh.conf["timers"]["timeout_timer_stamp"].isNull())
		m_timerout_Time = m_bDelayTime;
	else
		m_timerout_Time = zjh.conf["timers"]["timeout_timer_stamp"].asInt();

	if (!zjh.conf["robot"]["auto_out"].isNull())
		m_auto_out_trustee = zjh.conf["robot"]["auto_out"].asInt() == 1;
...
{	
	...
	log.info("%s player tid:%d uid:%d\n", __FUNCTION__, player->tid, player->uid);
	
	if (!val["resuser"].isNull())
	{
		m_Disband_ResUser = val["resuser"].asInt();
	}
	if (!val["ting_info"].isNull())
	{
		if (val["ting_info"].asInt() == 1)
			m_send_listen_info = true;
		else
			m_send_listen_info = false;
	}
}
...
void Table::Stand_Up(Player *player)
{
	...
	{
		log.error("%s uid:%d stand up but seatid:%d is wrong \n ", __FUNCTION__, player->uid, player->seatid);
		return;
	}
	log.info("%s uid:%d chairid:%d success\n", __FUNCTION__, player->uid, player->seatid);
...
void Table::Player_Leave(Player *player)
{
	if (player->seatid >= m_GAME_PLAYER || player->seatid < 0)
	{
		log.error("%s uid:%d leave but seatid:%d is wrong \n ", __FUNCTION__, player->uid, player->seatid);
		return;
	}
	log.info("%s uid:%d chairid:%d success\n", __FUNCTION__, player->uid, player->seatid);
	player->status = STAND_UP;
...
int Table::del_player(Player *player)
{
	if (player == NULL)
	{
		log.error("del_player, but player is NULL \n");
		return -1;
	}		
	if (!find(player->uid))
	{
		log.error("can not find uid:%d \n", player->uid);
		return -1;
	}
...
{  
	if (NULL == player)
	{
		log.error("handler_login_succ_uc error player is null");
		return -1;
	}
...
int Table::handler_table_info(Player *player)
{
	...
	stTableinfo.set_matchtype(match->m_MatchType);
	stTableinfo.set_pushgametype(match->m_PushGameTyep);
	unicast(SERVER_TABLE_INFO_UC, player, stTableinfo);	

	//log.info("match->AllTurn():%d match->CurTurn():%d match->AllRound():%d m_CurRound:%d stTableinfo = %s\n", match->AllTurn(), match->CurTurn(), match->AllRound(), m_CurRound, stTableinfo.DebugString().c_str());
	return 0;
}
...
{   
	if (m_bGameState != GAME_FREE || m_room_status != 0)
	{
		for (int i = 0; i < m_GAME_PLAYER; i++)
		{
			...
			{
				log.info("%s error uid:%d is in playing\n", __FUNCTION__, player->uid);				
				if (player->seatid >= 0 && player->seatid < m_GAME_PLAYER)
				{
					m_IsOffline[player->seatid] = true;//记录断线情况
				}	
...
{	
	...
	{
		log.error("%s tid:%d  ackchatbc parse error \n", __FUNCTION__, tid);
	}
	return 0;
}
...
void Table::Send_Game_Scene(Player *player)
{
	...
	//log.info("%s over to uid:%d chairid:%d \n", __FUNCTION__, player->uid, player->seatid);	
}
...
{  
	if (NULL == player)
	{   
		log.error("uptable_error_uc error player is null");
		return ;
	}
...
{  
	if (NULL == player)
	{
		log.error("handler_uptable player is null\n");
		return  0;
	}
	for (int i = 0; i < m_GAME_PLAYER; i++)
	{
	...
	if (player->money < stand_money) {
		log.error("%s error uid:%d money:%d too less stand_money:%d.\n", __FUNCTION__, player->uid, player->money, stand_money);
...
{  
	if (player == NULL)
	{
		log.error("handler_downtable error player is null.\n");
		return -1;
	}
...
void Table::OnUserGetCard(_uint8 ChairID, bool bGetLast)
{
	...
	log.info("%s tid:%d uid:%d chair:%d get:0x%02x optype:0x%x left:%d cardpoollen:%d left:%d\n",
...
bool Table::CheckUserOutCard(_uint8 ChairID, _uint8 cbCard)
{
	...
	if(ChairID != m_bCurChairID || cbCard == INVALID_MJ_CARD) 
	{
		...
		log.error("%s error cur ChairID:%d, recv out uid:%d chairID:%d card:0x%02x\n", __FUNCTION__, m_bCurChairID, GetPlayerUid(ChairID), ChairID, cbCard);		
		return false;
	}	
	if (m_bOutCardChairID == ChairID)
	{
		if (m_SeatPlayer[ChairID].player != NULL)
			Send_Game_Scene(m_SeatPlayer[ChairID].player);
		log.error("%s roomid:%d ChairID:%d  m_bOutCardChairID:%d\n", __FUNCTION__, tid, ChairID, m_bOutCardChairID);
		return false;
	}
	// 检查此张牌是否在用户手牌里
	...
	{
		_uint8* c = m_HandCard[ChairID].ChangeableCards;
		log.error("%s uid:%d seatid:%d outcard error left:%d,[0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x] \n",
...
bool Table::CheckUserOperator(_uint8 ChairID, int iOpType, _uint8 bCard)
{
	...
	{
		log.error("%s error chairid:%d iOpType:0x%x\n", __FUNCTION__, ChairID, iOpType);
		return false;
	}
	...
	{
		log.error("%s rob gang error uid:%d chairid:%d iOpType:0x%x m_bUserOperate:0x%x\n", __FUNCTION__, GetPlayerUid(ChairID), ChairID, iOpType, m_bUserOperate[ChairID]);
		return false;
	}
	return true;
}
...
void Table::GameEnd()
{
	...
	log.fatal("%s:%d:%s m_SeatPlayer[0].uid:%d m_SeatPlayer[1].uid:%d m_SeatPlayer[2].uid:%d m_SeatPlayer[3].uid:%d tid:%d round:%d score[%d,%d,%d,%d]\n",__FILE__,__LINE__, __FUNCTION__, m_SeatPlayer[0].uid, m_SeatPlayer[1].uid, m_SeatPlayer[2].uid , m_SeatPlayer[3].uid, tid, m_nPlayingTimes, hu_score[0].iTotleScore, hu_score[1].iTotleScore, hu_score[2].iTotleScore, hu_score[3].iTotleScore);
	...
	for (int  i = 0; i < m_GAME_PLAYER; i++)
	{
		...
		if (player != NULL)
		{
			...
			{
				settle.sRival.push_back(m_SeatPlayer[c].player->uid);
			}
		}
	}
...
{  
	...
	for (int i = 0; i < m_GAME_PLAYER; i++)
	{
		...
		m_SeatPlayer[i].player->m_zhigang = 0;
		m_SeatPlayer[i].player->m_wangang = 0;
		m_SeatPlayer[i].player->m_angang = 0;
	}

	log.info("%s:%d:%s mitchid:%d tid:%d uid:[%d,%d,%d,%d] game reset\n", __FILE__,__LINE__,__FUNCTION__, matchid,tid,
...
void Table::recv_client_outcard(Client *client, Player *player)
{
	if (player->seatid >= m_GAME_PLAYER || player->seatid < 0)
	{
		log.error("%s but uid:%d chaird:%d is wrong \n", __FUNCTION__, player->uid, player->seatid);
		return;
	}
	for (int i = 0; i < m_GAME_PLAYER; i++)
	{
	...
	log.error("%s  uid:%d is wrong \n", __FUNCTION__, client->uid);
	return;
__FUNC_BEGIN__:	
	if (client->m_Buffer == NULL)
	{
		log.info("%s buffer is null\n", __FUNCTION__);
		return;
	}
	...
	m_trustee.get()->UserOperate(bOutChairID, 0);
...
void Table::recv_client_opeatereuslt(Client *client, Player *player)
{
	if (player->seatid >= m_GAME_PLAYER || player->seatid < 0)
	{
		log.error("recv client Operate, but chaird:%d is wrong \n", player->seatid);
		return;
	}
	for (int i = 0; i < m_GAME_PLAYER; i++)
	{
...
__FUNC_BEGIN__:
	...
	//log.info("recv client operate, uid:%d, chairid:%d, optype:0x%x, card:0x%02x \n", player->uid, chairid, iOpType, bCard);
    if (m_bDeleteTable || m_bIsDisbandTable)
    {
		return;
	}

	m_trustee.get()->UserOperate(ChairID, 0);
...
void Table::RecvClientAutoHu(Client *client, Player *player)
{
	if (client == NULL)
	{
		log.error("user trustee but, client is NULL \n");
		return;
	}
...
{	
	...
	{
		tagListenInfos stListenInfo[HAND_CARD_SIZE_MAX];
		_uint8 bListenLen = 0;
...
void Table::UpdateUserInfoToUser(Player *player)
{
	...
	log.info("%s:%d:%s uid:%d chairid:%d money:%d\n",__FILE__,__LINE__, __FUNCTION__, player->uid, player->seatid, player->money);	
...
void Table::RecvUserTrustee(Client *client, Player* player)
{
	if (client == NULL)
	{
		log.error("user trustee but, client is NULL \n");
	...
	proto::game::AckIsAuto ack;
	ack.ParseFromArray(client->m_Buffer->proto_data, client->m_Buffer->proto_head.length);
	
	if (ack.isauto() == 0)
	{
		m_trustee.get()->UserOperate(player->seatid, 0);
	}
}
...
//手牌为14张牌的时候的判断听牌
int Table::GetListeningInfo(_uint8 ChairID, tagListenInfos stListenInfo[HAND_CARD_SIZE_MAX], _uint8& bListenLen)
{
	HandCards &tempHandCard = m_HandCard[ChairID];

	_uint8 bCard = 0xFF;
	memset(stListenInfo, 0, sizeof(tagListenInfos) * HAND_CARD_SIZE_MAX);
	bListenLen = 0;	
...
void Table::GetHandCardRelativity(_uint8 bCard, MJ_BUFFER<_uint8, POOL_CARD_TYPE_MAX>& _relativity)
{
	...
	else
	{
		...
		else if (CardNum(bCard) == 2)
		{
			_start = bCard - 1;
			_end = bCard + 2;
		}
		else if (CardNum(bCard) == 8)
		{
			_start = bCard - 2;
			_end = bCard + 1;
		}
...
void Table::Check_Listening(HandCards *pHc, _uint8 ChairID, _uint8 bCard, tagListenInfos stListenInfo[HAND_CARD_SIZE_MAX], _uint8 &bListenLen)
{
	if (bListenLen >= HAND_CARD_SIZE_MAX)
	{
		log.error("%s chaochu zuida ting pai shu!\n", __FUNCTION__);
		return;
	}
...
void Table::Send_lookon_GameScene(Player *player)
{
	if (player == NULL)
	{
		log.error("Send_lookon_GameScene, player is NULL \n");
		return;
	}
	int ChairID = player->seatid;	
...
void Table::save_video_palyer_info()
{
	...
	for (int i = 0; i < m_GAME_PLAYER; i++)
	{
		if (m_SeatPlayer[i].player)
		{   
			//log.info("save_video_palyer_info uid:%d chairID %d\n", GetPlayerUid(i), i);
			proto::login::tagPlayers *paddPlayer = stTableinfo.add_players();
			paddPlayer->set_uid(m_SeatPlayer[i].player->uid);
			paddPlayer->set_seatid(m_SeatPlayer[i].player->seatid);
...
void Table::save_user_get_card(_uint8 chairID, _uint8 cbCard, int nLeftCardNum)
{
	...
	//log.info("save_user_get_card uid:%d chairID:%d card:0x%02x \n", GetPlayerUid(chairID), chairID, cbCard);
