...
// 		std::string tid = to_char(table->tid);
// 		std::map<std::string, Table*>::iterator iter2 = all_tables.find(tid);
// 		if( iter2 != all_tables.end() )
// 		{
// 			all_tables.erase( iter2 );
// 		}
// 		log_info("%s delete table! tid:%d left tables:%d \n", __FUNCTION__, table->tid, all_tables.size());
...
int Game::init_accept()
{
	...
	if (setsockopt(_fd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) < 0) {
		log_error("File[%s] Line[%d]: setsockopt failed: %s\n", __FILE__, __LINE__, strerror(errno));
		close(_fd);
		exit(1);
		return -1;
	}
	...
	if (bind(_fd, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
		log_error("File[%s] Line[%d]: bind failed: %s\n", __FILE__, __LINE__, strerror(errno));
		close(_fd);
		exit(1);
		return -1;
	}
...
void Game::accept_cb(struct ev_loop *loop, struct ev_io *w, int revents)
{
	...
	{		
		...
		log_info("%s Client connect ip:%s port:%d\n", __FUNCTION__, client->string_ip.c_str(), client_addr.sin_port);
	} 
	else
	{
		close(fd);
...
//	assert(client->m_Buffer != NULL);
	...
	log_info("%s tid:%s uid:%d zid:%d skey:%s\n",__FUNCTION__, client->tid_str.c_str(), uid, server_zid, client->skey.c_str());
	if (uid == 0)
	{
		return -1;
	}
	...
	if( uid_client_map.find( uid ) != uid_client_map.end() )
	{
		...
		{
			log_info("%s mapSession tid:%s uid:%d\n", __FUNCTION__, strroomid.c_str(), uid);
	...
	//请求玩家信息
	log_info("begin req player info uid=%d \n", uid);
...
int Game::login_succ_back(Json::Value &val )
{
	...
	log_debug("%s success, uid:%d usertype:%d \n", __FUNCTION__, uid, usertype);
	...
	if (all_tables.find(client->tid_str) != all_tables.end())
	{
		...
		if (table->m_mapTable_Player.find(client->uid) != table->m_mapTable_Player.end())
		{
			...
			log_error("%s rebind tid:%s uid:%d get info ok\n", __FUNCTION__, client->tid_str.c_str(), uid);
			del_client(client);
			return 0;
		}
	}
	...
	log_info("%s new player uid:%d success. save point client:0x%x\n", __FUNCTION__, uid, client);
	//如果是机器人，加到机器人队列中
// 	if (usertype == 4 )
// 	{
...
void Game::del_client(Client *client, bool del_oldclient)
{
	...
	if (del_oldclient)
	{
		log_info("%s delete old client fd:%d uid:%d\n", __FUNCTION__, client->fd, client->uid);
	...
	{
		log_error("%s del client free fd[%d].\n", __FUNCTION__, client->fd );
		return;
	}	
	...
	if (client->uid != -1)
	{
		...
		{
			log_info("%s uid:%d erase uid_client_map\n", __FUNCTION__, client->uid);
			uid_client_map.erase(client->uid);
		}
	}
	if (client->player)
				...
				if (table->m_mapTable_Player.find(uid) != table->m_mapTable_Player.end())
				{
					...
					if (player != NULL)
					{
						...
						else
						{
							player->client = NULL;
							game_user_offline(player);
							log_info("%s player is null offline tid:%s uid:%d \n", __FUNCTION__, client->tid_str.c_str(), uid);
						}						
					}
				}
			}
		}
		log_info("%s player is null end tid:%s uid:%d\n", __FUNCTION__, client->tid_str.c_str(), client->uid);
	}

	if (client)
	{
		log_info("%s uid:%d del client fd:%d 0x%x\n", __FUNCTION__, client->uid, client->fd, client);
		delete client;
		client = NULL;	
	}
}
	...
	switch (cmd)
   	{
		...
		case CLIENT_HOME_OUT:
			...
			break;
		case CLINET_MJ_CHECK_USER_HAND_CARD:
		{//校验手牌是否正确。
			all_tables[player->tid_str]->recv_client_checkuserhandcard(client, player);
		}
		break;
...
{	
	...
	case SERVER_VIDEO_DATA_ERR_RESP:
	{
		...
		log_debug("save vedio data resp roomID:%d, num:%d, status:%d, datetime:%d\n", roomID, nNum, status, ndatatime);
	}
	break;
...
// 		log_error("safe check uid[%d] must be online player.\n", client->uid);
// 		return -1;
// 	}
// 
// 	Player *player = client->player;
// 	if (player == NULL) {
// 		log_error("safe check client player is NULL.\n");
// 		return -1;
// 	}
// 	if (all_tables.find(player->tid_str) == all_tables.end())
// 	{
// 		log_error("safe_check tid:%s uid:%d is not in table\n", player->tid_str.c_str(), player->uid);
// 		return -1;
// 	}
// 
// 	return 0;
...
// 			}
// 		}
		log_info( "%s succ tid:%s uid:%d \n", __FUNCTION__, roomid.c_str(), player->uid);
		return 0;
	}

	log_error( "%s succ uid:%d roomid is NULL\n", __FUNCTION__, player->uid );
	return -1;
}
...
{   
	...
	if (all_tables.find(player->tid_str) != all_tables.end()) 
	{
		...
		if (ret < 0) {
			log_error("%s table handler logout\n", __FUNCTION__);
			return -1;
		}
		if( NULL == player )
		{
	...
	if (player)
	{
		...
		log_info("%s delete player! tid:%s uid:%d player:%p\n", __FUNCTION__, player->tid_str.c_str(), uid, player);
		delete player;
		player = NULL;
	}
...
int Game::dz_request_login( Client *client )
{
	...
	if( uid_client_map.find( uid ) != uid_client_map.end() )
	{
		if( uid_client_map[uid]->fd != client->fd )
		{
			log_info("%s tid:%s uid:%d client player delete uid_client_map\n", __FUNCTION__, client->tid_str.c_str(), client->uid);
			del_client( uid_client_map[ uid ] );			
		}
	}
...
int Game::dz_login_reconnect(Client* client, Player* player)
{
	...
	//uid_client_map[uid] = client;
	log_info("%s tid:%s uid:%d success\n", __FUNCTION__, client->tid_str.c_str(), client->uid);
	return 0;
}
...
{   
	...
	{
		close(fd);
		log_error("connect video svr error: %s(errno: %d)\n", strerror(errno), errno);
		fd = -1;
		exit(1);
	}
	fcntl(fd, F_SETFL, fcntl(fd, F_GETFL, 0) | O_NONBLOCK);
	...
	if( NULL != link_video_client )
	{
		log_info("connect video svr fd[%d].\n", fd );
		//		fd_client_map[link_data_client->fd] = link_data_client;
		//register_server();
	}
	else
...
int Game::reconnect_videosvr()
{
	...
	{
		close(fd);
		log_error("reconnect videosvr error: %s(errno: %d)\n", strerror(errno), errno);
		return -1;
	}
	fcntl(fd, F_SETFL, fcntl(fd, F_GETFL, 0) | O_NONBLOCK);
	link_video_client = new (std::nothrow) Client( fd, false, 1);
	...
	{
		log_info("%s videosvr link fd[%d].\n", __FUNCTION__, fd);
		//fd_client_map[link_data_client->fd] = link_data_client;
		ev_timer_stop(zjh.loop, &ev_reconnect_videosvr_timer );
		//register_server();
	}
	log_info("%s reconnect!\n", __FUNCTION__);
	return 0;
}
...
int Game::handler_room_conf_res(Json::Value &val)
{
	...
	if (it != tid_client_map.end()) {
		//获取配置信息失败
		if (status != 200) {
			log_error("%s room[%s] conf res error\n", __FUNCTION__, roomid.c_str());
...
void Game::SendLogout(Client *client, int uid, int code)
{
	...
	log_info("%s uid:%d loging error code:%d logout out room\n", __FUNCTION__, uid, code);	
...
void Game::player_offline_timeout(Player *player)
{
	...
	log_info("%s tid:%d player is timeout uid:%d \n", __FUNCTION__, player->tid_str.c_str(), player->uid);
	if (all_tables.find(player->tid_str) != all_tables.end())
	{
