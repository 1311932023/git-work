...
void Table::GameEnd()
{
	...
	{
		//GameEndAndGetMaNum(m_UserOperateHuCard);
...
void Table::recv_client_outcard(Client *client, Player *player)
{
	...
	bool   bOutCardIsLaiFlag = stOutCard.hongzhonglaiziflag();
	...
	{	
		if (bOutCardIsLaiFlag==true)
		{
			OnUserOutCard_XianNing(bOutChairID, bOutCard);
		}
		else
		{
...
{
	log_info("RobGangPriority start");
	stop_operator_timer(ChairID);
	...
	if (m_RobOperateNum == m_RobHuNum)  //所有胡玩家都已经操作完毕
	{
		if (m_bRobHu)   //如果有人胡牌
		{
			if (!m_conf.m_bYiPaoDuoXiang)
			{
				...
				{
					log_info("RobGangPriority m_bUserOperate[c = %d] = %x",c, m_bUserOperate[c]);
