...
{	
	log.info("%s:%d:%s zjh.game->qualifier_match:%d\n", __FILE__, __LINE__, __FUNCTION__, zjh.game->qualifier_match);
	...
	if (zjh.game->qualifier_match)
		m_TableConfigInfo["dataMap"]["innerWayList"] = remote;

	//in.close();

	//log.info("%s:%d:%s m_TableConfigInfo:%s\n", __FILE__, __LINE__, __FUNCTION__, m_TableConfigInfo.toStyledString().c_str());

	return m_TableConfigInfo;
}
