...
#include "proto/hunangameproto.pb.h"
...
typedef enum tagGameStatus
{
	...
	//GAME_PIAO = 5,            //选票场景
	GAME_PIAO = 4,                //选票场景
	GAME_QISHOU_SEL_BAI_PAI  = 5 ,				// 摆牌 阶段 
	GAME_QISHOU_BAOTING_SEL = 6 ,  // 开始选择 报听 
	GAME_QISHOU_BAOTING_OVER = 7 ,
...
public:
	...
	ev_timer                    send_videodata_timer;  //11
	ev_tstamp                   send_videodata_timer_stamp;

	ev_timer                    req_start_timer;     // 22
...
public:
	 ...
	 *  @return:
	 **/
	void recv_client_opeater_req(Client *client, Player *player);
	//收到自动胡牌协议
	...
	//接受客户端，，摆牌的 消息 
	//void recv_client_baipai_sel(Client *client, Player *player);
	void recv_client_baoting_sel(_uint8, int);
...
protected:
	...
	//出牌时钟
	ev_timer                    m_Outcard_Timer;   //  44
	ev_tstamp                   m_Outcard_Timer_stamp;
	//操作时钟
	ev_timer                    m_Operate_Timer0;   // 55 
	ev_tstamp                   m_Operate_Timer_stamp0;

	ev_timer                    m_Operate_Timer1;    //66 
	ev_tstamp                   m_Operate_Timer_stamp1;

	ev_timer                    m_Operate_Timer2;    //77 
	ev_tstamp                   m_Operate_Timer_stamp2;

	ev_timer                    m_Operate_Timer3;   //  88 
	ev_tstamp                   m_Operate_Timer_stamp3;

	ev_timer					m_CurTimer;	//当前开启的定时器   //  99 

	//等待玩家选择是否解散房间时钟
	ev_timer                    m_Disbandtable_Timer0;    //10 10 
	ev_tstamp                   m_Disbandtable_Timer_stamp;
	ev_tstamp                   m_Disbandtable_Timer_stamp1;

	ev_timer					m_RecvDisbandTable_Timer;   //  11 11 
...
public:
	...
	short                 m_nGameRunState;                  // 游戏进行到的阶段。。

	int                 m_nCanBaoTingArr[GAME_PLAYER];
	int                  m_nRecvBaoTingPlayerMap[GAME_PLAYER];
	MJ_hand_buffer		 m_qishou_banpai[GAME_PLAYER];
	int					m_RecvBaipaiMap[GAME_PLAYER];
	int                  m_CanBaiPaiArr[GAME_PLAYER];

	bool                 m_canBankCanOpCard;
...
public:
	...
	_uint8              m_bGrapId; 
	int                 m_PlayGrapCard[GAME_PLAYER];      //  1  

	ShowCards           m_stUserOutCard[GAME_PLAYER];	 //各个玩家打出的牌    
	ShowCards           PlayerAllOutCard[GAME_PLAYER];	 //各个玩家打出的牌    
	int                 m_bBaoTingOutCount[GAME_PLAYER]; 


	ShowCards           m_stUserOutCardAfterBaoTing[GAME_PLAYER];	 // 报听后打出去的牌。 ！！ 宁乡麻将。杠后。。打出去的几张牌，不能吃碰刚
...
protected:
	...
	//MJ_fan_buffer          m_specialFan[GAME_PLAYER];
...
public:
	 ...
	 *	玩家摸牌
	 */
	virtual void OnUserGrapCard(_uint8 ChairID, bool bGetLast = false);
	/*
	 *	玩家打牌
	 */
	...
	*/
	 ...
	 *	玩家出牌后判断其他玩家是否能够吃碰胡
	 */
	virtual int CheckOutCardOtherPlayerCanOpCode(_uint8 bOutChairID, _uint8 bCard, MJ_major& major, int& fan_num);
	/*
	*	玩家摸牌后判断是否能够杠听胡
	*/
	virtual int CheckPlaySlefGrpCardCanOperateCode(_uint8 ChairID, _uint8 bGetCard, MJ_major& major, int& fan_num);
	/*
	 *	检查牌有没有胡
	 */
	virtual int Check_Hu(_uint8 ChairID, HandCards& pHc, MJ_fan_buffer& oFan, _uint8  add_last_card = 0);
	/*
	 *	判断牌是不是7对
	 */
	...
	*/
	 ...
	 *	胡操作
	 */
	virtual int  UserOperateHu(_uint8 ChairID, _uint8 bHuCard = 0xFF);
	/*
	 *	过操作
	 */
	 ...
	 *	玩家执行杠操作
	 */
	virtual int  UserOperateGang(_uint8 ChairID, int iOpType, _uint8 bCard);
	/*
	 *	玩家吃
	 */
	...
	//检查听牌
	virtual bool Check_Listening(HandCards *pHc, _uint8 ChairID, _uint8 bCard, tagListenInfos stListenInfo[14], _uint8 &bListenLen); 

	virtual void  Reconnect(int sid){};
	...
	virtual  void  OnRecvQishouHuBaiPai_Operate(int sid, int op){};
	virtual  void  OnRecvBaoTingOp(int sid, int  op){};
	virtual  void  DealNingxiangHuBroad(_uint8  chairid, MJ_fan_buffer  & fan){};

	virtual  int   check_guilin_listen_xilun(_uint8 sid){ return 0; };  
...
public:
	...
	virtual void Calc_Hu_Player_Base_Score(_uint8 ChairID, int iBaseScore[GAME_PLAYER]){};
	...
	virtual  void DealGuiLinJieSuan(proto::game::AckGameEnd& stGameend) {};
...
public:
	...
	Player *GetPlayer(_uint8 ChairID )
	{
		if ((ChairID < m_GAME_PLAYER) && (m_SeatPlayer[ChairID].player != NULL))
			return   m_SeatPlayer[ChairID].player;
	...
	int check_player_can_bao_ting(_uint8 sid , int end_pos = 34  );
	
	bool Check_Listening_haohuaqidui(HandCards *pHc, _uint8 ChairID, _uint8 bCard, tagListenInfos stListenInfo[14], _uint8 &bListenLen);
	int deal_hu(_uint8 ChairID);
	bool check_listening_guilin_xilan(HandCards *pHc, _uint8 ChairID, _uint8 bCard, tagListenInfos stListenInfo[14], _uint8 &bListenLen); 
	void recv_jilin_majiang_gang_req(Client *client, Player *player);


	virtual  int  ProtoJilinGangOperate (int, int, int *arr, int len, int gang_id){ return 0; };
	virtual  int  ProtoNingXiangFengDong(int op) { return 0; };
	void Proto_OutCardErr(_uint8 chairID, _uint8 card, int err_code);
	void recv_ningxiang_fengdong_op(Client *client, Player *player);
	int GetNextBanker();
	int Get_Hand_All_Card_Count(const HandCards &phc, _uint8 cur_card);
	int GetZhongfabaCount(HandCards *phc);
	//bool Check_Listening_zhongfabai(HandCards *pHc, _uint8 ChairID, _uint8 bCard, tagListenInfos stListenInfo[14], _uint8 &bListenLen);
...
int Table::unicast(int iMsg, Player* player, const TYPE& p)
{
	...
	int bodysize = p.ByteSize();
	int headsize = PackHeader(buf, iMsg, bodysize);	
	if ((headsize + bodysize) >= static_cast<int>(sizeof(buf)))
		return -1;
	//assert((headsize + bodysize) < static_cast<int>(sizeof(buf)));
	p.SerializeToArray(buf + headsize, bodysize);
	if ((player != NULL) && (player->client != NULL))
