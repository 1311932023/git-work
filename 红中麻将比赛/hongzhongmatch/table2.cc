...
{	
	...
	if (m_conf.m_bCheckCardNum && pHc->ChangeableCardsLen % 3 != 1)
	{
		...
		log.error("%s cards(%d):%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x\n", __FUNCTION__, pHc->ChangeableCardsLen,
...
void Table::DoMultiOperate(_uint8 ChairID, int iOpType, _uint8 bCard)
{
	...
	log.info("%s tid:%d uid:%d chaird:%d, curoptype:0x%x, optype:0x%x, now done OpNum:%d, totalOpnum:%d \n", __FUNCTION__, tid,
...
int Table::dispatch_client(int cmd, Client *client, Player *player)
{
	...
	{
		log.error("%s tid:%d client has disbanded \n", __FUNCTION__ , tid);
		return -1;
	}
	if (m_bDeleteTable)
	{
		log.error("%s tid:%d table has deleted cmd:%d uid:%d \n", __FUNCTION__, tid, cmd, GetPlayerUid(player));
		return 0;
	}
	...
	case CLIENT_MJ_HOME_RETURN:
		log.info("Recv_Home_Return req tid:%d uid:%d\n", player->tid, player->uid);
		handler_table_info(player);
		Send_Game_Scene(player);
		return 0;
	case CLIENT_MJ_TRUSTEE:
		RecvUserTrustee(client, player);
//		log.info("recv trustee tid:%d uid:%d\n", player->tid, player->uid);
		return 0;
	case CLIENT_MJ_AUTOHU:
		RecvClientAutoHu(client, player);
		log.info("recv auto hu tid:%d uid:%d\n", player->tid, player->uid);
		return 0;
	...
	log.error("%s invalid command[%d], tid:%d uid:%d\n", __FUNCTION__, cmd, tid, player->uid);
	return -1;
}
