...
//BOOST_CLASS_EXPORT_GUID(MatchExt, "MatchExt")
...
void MatchExt::CalculateCurRank(Player *player, int curRound)
{
	if (curRound != m_CurRound) {
		log_error("not current round settle current[%d] your[%d]\n", m_CurRound, curRound);
		return;
	}
	//非最后一轮若排名相同对比时间
	...
	log_info("%s player SettleInfo count:%d\n", __FUNCTION__, player->m_SettleInfo.size());	
}

void MatchExt::CalculateTotalRank(std::map<int, Player*>& map_players)
{
	tmpRank.clear();
	log_info("%s mid:%d turn:%d round:%d\n", __FUNCTION__, matchid, m_CurTurn, m_CurRound);
	...
	for (std::vector<RankInfo>::iterator it = tmpRank.begin(); it != tmpRank.end(); ++it) {
		...
		if (map_players.find(uid) != map_players.end()) {
			...
			player->UpdateLosePlayersMpRatio();
			it->totalRank = i;
			++i;
		}
	}
}
void MatchExt::CalculateTotalRank3()
{
	tmpRank.clear();
	log_info("%s mid:%d turn:%d round:%d\n", __FUNCTION__, matchid, m_CurTurn, m_CurRound);
	std::map<float, int> rankcount;
...
int MatchExt::StatisticGameResult(Table *table)
{
	...
	if (all_players.find(player->uid) == all_players.end()) {
		log_error("%s player[%d] not in match[%d]\n", __FUNCTION__, player->uid, matchid);
		return -1;
	}
	...
	log_info("%s mid:%d uid:%d round:%d OverTable:%d\n",__FUNCTION__,
...
void MatchExt::StatisticSeatRank(int seatid, int cur_round)
{
	...
	for (std::map<int, std::multimap<float, Player*> >::iterator it = tempScore.begin(); it != tempScore.end(); ++it) {
		...
		for (std::multimap<float, Player*>::iterator it_p = it->second.begin(); it_p != it->second.end(); ++it_p) {
			...
			++j;
//			log_info("%s mid:%d uid:%d round:%d cur_rank:%d mpRank:%d score:%d ratio:%f\n", __FUNCTION__, matchid, p->uid, cur_round, p->m_SettleInfo[cur_round].sCurRank, p->m_SettleInfo[cur_round].sMPRatioRank, p->m_SettleInfo[cur_round].sMPScore, p->m_SettleInfo[cur_round].sMPRatio);
		}
		++i;
	}
}
...
void MatchExt::EliminateRobot()
{
	...
	if (giveupNum >= all_robots.size()) {
		robotgiveupNum = all_robots.size();
	}
	else {
	...
	while (robotgiveupNum > 0) {
		std::set<Player*>::iterator it = all_robots.begin();
		(*it)->m_GiveUp = true;
		all_robots.erase(it);
		robotgiveupNum--;
	}	
...
int MatchExt::UploadMPInfo()
{
	...
	for (size_t i =0; i < m_roundinfo.size(); )
	{
		Json::Value& tmp = m_roundinfo[i++];
		int uid = tmp["uinfo"]["player_id"].asInt();
		int roundid = tmp["rinfo"]["round"].asInt();
		if (all_players.find(uid) != all_players.end())
		{
	...
	if (ret < 0) {
		log_error("redis set mid[%d] mp error\n", matchid);
		return -1;
	}
//	log_info("%s mid:%d val:%s\n", __FUNCTION__, matchid, tmp.c_str());
