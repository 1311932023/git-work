...
void Table::Proto_Game_OpNotifyAfterPengChi(_uint8 ChairID, _uint8 CurChairID, const MJ_opinfo& gang, proto::game::AckOpNotify& stNotify)
{
	...
	if (CurChairID == ChairID)
	{
		...
		for (int i = 0; i < gang.Size(); i++)
		{
			...
			log_info("set_card(gang[%d].cbCard) = %x", i, gang[i].cbCard);
...
int Table::dispatch_client(int cmd, Client *client, Player *player)
{
	...
	{	
		proto::game::yunxiaomj::GameEndRecord ger;		
		for (int i = 0; i < m_GAME_PLAYER; i++)
		{
...
void Table::recv_client_checkuserhandcard(Client *client, Player *player)
{
	...
	int bChairID = player->seatid;
	proto::game::ReqCheckHandCards stHandCard;
	stHandCard.ParseFromArray(client->m_Buffer->proto_data, client->m_Buffer->proto_head.length);
	_uint8 bHandCards[20] = { 0x0 };
	int cards_len = 0;
	for (int i = 0; i < stHandCard.cards_size();i++)
	{
		if (cards_len<20)
		{
			bHandCards[cards_len++] = stHandCard.cards(i);
		}
	}
	//log_info("%s uid:%d chairid:%d outcard:0x%02x \n", __FUNCTION__, player->uid, bOutChairID, bOutCard);
	if (m_bDeleteTable || m_bIsDisbandTable)
	{
		return;
	}
	CheckUserHandCards(bChairID, bHandCards, cards_len);
	//检查玩家打出的牌
