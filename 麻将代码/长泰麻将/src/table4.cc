...
void Table::Fill_FixedCard_AnGang(_uint8 OpChairID, _uint8 ChairID, _uint8 Pos, proto::game::FixedCard* pstFix)
{
	_uint8 cbCard = 0xFF;
	...
	{
		if (m_HandCard[ChairID].FixedCards[Pos].state == TYPE_ANGANG)
		{
			cbCard = 0x00;
		}
		else
	...
	//log_info(" %s uid:%d ChairID:%d pstFix = %s\n", __FUNCTION__, GetPlayerUid(ChairID), ChairID, pstFix->DebugString().c_str());
}
