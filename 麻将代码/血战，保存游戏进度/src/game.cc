...
void restartProc(void)
{
	...
	/*std::ofstream ofs(filename.c_str());*/
	{
		...
		oa << BOOST_SERIALIZATION_NVP(zjh);
	}
	archiveStr = archiveStream.str();
	int ret = zjh.main_rc[0]->command("set archiveStr:%d %s", 1, archiveStr.c_str());
	if (ret < 0) {
		printf("redis set archiveStr error\n");
	}
	/*ofs.flush();
	ofs.close();*/
...
void Game::AcceptCB(struct ev_loop *loop, struct ev_io *w, int revents)
{
	...
	if( NULL == game->link_data_client ){
		close(fd);
		log_error("accept client error for the link of datasvr disconnect.\n");
		return;
	}
	...
	log_info("%s fd:%d ip:%s port:%d\n", __FUNCTION__, fd, inet_ntoa(client_addr.sin_addr), htons(client_addr.sin_port));
...
void Game::SendToDataSvr( const std::string &res )
{
	if( NULL != link_data_client ){
		...
		log_error("link data svr is null\n");
	}
}
...
void Game::DelClient(Client *client, bool del_oldclient)
{
	...
	if ( link_data_client && client->fd != link_data_client->fd && (del_oldclient /*|| client->cmd_type == 1*/)){
		log_info("delete old client, fd:%d \n", client->fd);
	...
	if ( fd_client_map.find(client->fd) == fd_client_map.end() ){
		log_error("del client free client err[miss] fd[%d].\n", client->fd );
		if (uid_client_map.find(client->uid) != uid_client_map.end()) {
			if (uid_client_map[client->uid]->fd == client->fd)
			{
				...
				log_error("%d free client err[miss] fd[%d].\n", __FUNCTION__, client->fd);
			}
		}
		return;
	}
...
void Game::DelAllClient()
{
	...
	log_info("for disconnect to datasvr, delete all client fd_client_len[%d] temp_mFdClientMap[%d]  .\n", fd_client_map.size(), temp_mFdClientMap.size() );
...
{	
	...
	switch(cmd){
		...
		case SERVER_USER_EXCEPTION_ACK:
			log_info("reciev data svr res cmd:%d\n", cmd);
			return 0;
	}
	return 0;
}
...
{	
	...
	Player *player = client->player;
	if (player == NULL) {
		log_error("%s safe check cmd:%d\n", __FUNCTION__, cmd);
		return -1;
	}
	if (client->player != NULL)
	{
		log_info("%s mid:%d uid:%d cmd:%d\n", __FUNCTION__, player->matchid, player->uid, cmd);
	}
	
	if (all_match_map.find(player->matchid) == all_match_map.end()){
				
		log_error("%s safe_check uid:%d is not in match:%d\n", __FUNCTION__, player->uid, player->matchid);
		return -1;
	}
	...
	if(table == NULL){
		log_error("safe_check uid[%d] table is null\n", player->uid);
		return -1;
	}
	last_table = table;
...
int Game::JoinMatchReq(Client* client)
{
	...
	{	
		log_error("pReqLogin parse error\n");
		return -1;
	}
...
int Game::JoinMatch(int matchid, int uid, Client *client)
{
	...
	{
		log_error("%s find deleted match:%d\n", __FUNCTION__, matchid);
		return -1;
	}
	if(all_match_map.find(matchid) == all_match_map.end()){
		log_error("%s can't find match:%d\n", __FUNCTION__, matchid);				
		return -1;
	}
	...
	if (period_match)
	{
		...
		if (oldClient != NULL && oldClient->fd != client->fd) {
			log_info("%s uid:%d new fd:%d old fd:%d\n", __FUNCTION__, player->uid, player->client->fd, oldClient->fd);
			SendLogout(oldClient, client->uid, -1);
			oldClient->player = NULL;
			Client::pre_destroy(oldClient, true);
		}
		log_info("%s mid:%d uid:%d\n", __FUNCTION__, matchid, uid);
		if (table->m_bGameState == GAME_FREE && table->m_CurRound >= table->match->AllRound())
		{
...
int Game::MatchStart(Json::Value &val)
{
	...
	if(ret < 0){
		log_error("%s get minfo:%d error\n", __FUNCTION__, tmpMid);
		return -1;
	}
	if (zjh.main_rc[0]->reply->str == NULL)
	{
		log_error("%s mid:%d replay str is null\n", __FUNCTION__, tmpMid);
		return -1;
	}
	...
	if(reader.parse(strminfo, mval) < 0){
		log_error("%s parse minfo[%d] error\n", __FUNCTION__, tmpMid);
		return -1;
	}	
	log_debug("%s get redis minfo %s\n", __FUNCTION__, mval.toStyledString().c_str());
	...
	if (all_match_map.find(tmpMid) == all_match_map.end()) {
		...
		log_error("%s match:%d already exsit\n", __FUNCTION__, tmpMid);
	}
	return 0;
}
...
int Game::HandlerPrize(Json::Value &val)
{
	...
	Json::Value mval;
	if(reader.parse(strminfo, mval) < 0){
		log_error("%s parse prize_info[%d] error\n", __FUNCTION__, mid);
		return -1;
	}
	log_debug("%s get redis prize_info:%s\n", __FUNCTION__, mval.toStyledString().c_str());
	...
	for(size_t i = 0; i < mval.size(); ++i){
		...
		if(match->all_players.find(uid) == match->all_players.end()){
			log_error("%s cannot find player[%d]\n", __FUNCTION__, uid);
			continue;
		}
...
int Game::HandlerMatchPause(Json::Value &val)
{
	int mid = val["mid"].asInt();
	if(all_match_map.find(mid) == all_match_map.end()){
		log_error("HandlerMatchPause error not has mid[%d]\n", mid);
		return 0;
	}
	Match *match = all_match_map[mid];
...
void Game::SendLogout(Client *client, int uid, int code)
{
	...
	log_info("%s uid:%d code:%d\n", __FUNCTION__, uid, code);
}

int Game::MatchEndRes(int mid)
{
	if (all_match_map.find(mid) == all_match_map.end()) {
		log_error("%s match:%d end not exsit\n", __FUNCTION__, mid);
		return -1;
	}
...
void Game::restart()
{
	for (std::map<int, Match*>::iterator it = all_match_map.begin();
		it != all_match_map.end(); it++)
	{
		it->second->restart();
	}
	if (prize_timer_running)
	{
		TIMER_AGAIN(prize_timer, prize_tstamp);
	}
}